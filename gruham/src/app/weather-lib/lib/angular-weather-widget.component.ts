...
export class AngularWeatherWidgetComponent implements OnInit {
    ...
    if (this.APIKEY) {
      this.ws.getWeatherDetails(isGeoLocation,this.location, this.APIKEY).subscribe(response => {
        this.weatherDetails = {
          ...
          temperatureInCelsius:this.convertFahrenheitToCelsius(response.main.temp),
  ...
  convertFahrenheitToCelsius(fahrenheitTemp){
      var celsiusConvertedValue = 0;
      celsiusConvertedValue = ((fahrenheitTemp - 32) * 5)/9;
      celsiusConvertedValue = (Math.round(celsiusConvertedValue * 100) / 100);
      return celsiusConvertedValue.toString();
    }
}
